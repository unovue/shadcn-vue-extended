...
<template>
  <div :class="cn('flex h-full text-ink group-data-[view=preview]/block-view-wrapper:hidden', props.inContainer ? '' : 'mat-well overflow-hidden rounded-[12px] my-4 not-prose h-120')">
    <div :class="cn('w-70', props.inContainer ? '' : 'w-56')">
      <div class="min-h-full w-full flex flex-col">
        <div class="flex h-full flex-col w-full flex-1 border-r border-hairline bg-sub text-ink">
          <div class="duration-200 flex shrink-0 items-center font-medium outline-none ease-linear h-12 rounded-none border-b border-hairline px-4 text-[12.5px] text-ink">
            Files
          </div>
          <TreeRoot
            v-slot="{ flattenItems }"
          ...
          >
            ...
            >
              <Button
                ...
                class="flex w-full justify-start whitespace-nowrap rounded-none pl-[--index] hover:bg-well hover:text-ink focus-visible:bg-well focus-visible:text-ink active:bg-well active:text-ink data-[active=true]:bg-well dark:data-[active=true]:bg-well data-[active=true]:text-ink"
    ...
    <div class="flex min-w-0 flex-1 flex-col">
      <div class="flex h-12 flex-shrink-0 items-center gap-2 border-b border-hairline bg-sub px-4 text-[12.5px] font-medium text-ink">
